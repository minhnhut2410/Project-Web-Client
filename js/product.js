...
if (localStorage.getItem("filter")) {
    window.addEventListener("load", function () {
        const savedCategory = localStorage.getItem("filter");

        if (savedCategory) {
            currentCategory = savedCategory.toLowerCase().trim();

            filterButtons.forEach(btn => {
                btn.classList.toggle("active", btn.getAttribute('data-filter') === savedCategory);
            });
